 ...
 *
 * Wraps `eachMessage` processing with retry tracking. After `DLQ_MAX_RETRIES` consecutive failures,
 * the message is forwarded to a per-service DLQ topic
...
function withDLQ(producer, dlqTopic, logger, handler) {
  ...
  return async ({ topic, partition, message }) => {
    ...
    const attempt = (retryMap.get(messageKey) || 0) + 1;

    retryMap.set(messageKey, attempt);
    ...
    try {
      ...
      if (attempt >= DLQ_MAX_RETRIES) {
        ...
        // re-throw the error so KafkaJS retries (it will re-deliver the same message)
        throw error;
      }
    }
  };
